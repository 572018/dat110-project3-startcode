...
public class ChordLookup {
	...
	public NodeInterface findSuccessor(BigInteger key) throws RemoteException {
		NodeInterface highest_pred;

		// ask this node to find the successor of key
		// can't do this. It will create a loop...
		...
		if (Util.computeLogic(key, this.node.getNodeID().add(BIone), successor.getNodeID())) {
			// if logic returns true, then return the successor
			return stub;
		} else {
			// if logic returns false; call findHighestPredecessor(key)
			highest_pred = findHighestPredecessor(key);
		}
	...
	private NodeInterface findHighestPredecessor(BigInteger key) throws RemoteException {
		...
		for (int i = fingerTable.size()-1; i >= 0; i--) {
			...
			// check that finger is a member of the set {nodeID+1,...,ID-1} i.e.
			// (nodeID+1 <=finger <= key-1) using the ComputeLogic
			if (Util.computeLogic(stub.getNodeID(), this.node.getNodeID().add(one), key.subtract(one))) {
				// if logic returns true, then return the finger
				// (means finger is the closest to key)
				return stub;
			}
		}
