...
public class MutualExclusion {
	...
	public boolean doMutexRequest(Message message, byte[] updates) throws RemoteException {
		...
		boolean repliedAll = areAllMessagesReturned(activenodes.size());
		if (repliedAll) {
			// if yes, acquireLock
			node.acquireLock();
		...
		case 2: {
			...
			if (msgClock == clock) {
				if (message.getNodeID().compareTo(this.node.getNodeID()) < 0) {
					acknowlegdeMessage(message);
				}
			}
			// if sender wins, acknowledge the message, obtain a stub and call
			// onMutexAcknowledgementReceived()
			// if sender looses, queue it
			boolean useless = msgClock < clock ? acknowlegdeMessage(message) : this.mutexqueue.add(message);
			break;
		}
	...
	private boolean acknowlegdeMessage(Message message) {
		message.setAcknowledged(true);
	...
	private boolean areAllMessagesReturned(int numvoters) throws RemoteException {
		...
		boolean r = queueack.size() == numvoters ? true : false;
		// clear the queueack
		queueack.clear();
		return r;
	}
